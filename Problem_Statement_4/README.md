# 🚗✈️ Car vs. Plane Image Classification in R with Keras

## Overview

This experiment explores how a **neural network built in R** can be trained to distinguish between **car** and **plane** images.

The workflow is implemented in **Google Colab** and follows the standard pipeline for a simple binary image-classification task, including dataset loading, image preparation, feature extraction, model creation, optimization, prediction, and result checking.

Because the dataset is very small, this project is best seen as a learning example that shows the end-to-end process rather than a final production-grade computer vision system.
...
## ⚠️ Reading the Results Carefully

A test accuracy of **50%** should not be taken as proof that the model is fundamentally unable to separate cars from planes.

The real limitation here is the tiny dataset. Only **10 images** were used for training, and just **2** were kept for testing. With more than **635,000 trainable parameters**, the model can easily fit the training examples without learning meaningful visual patterns that generalize well.

This makes overfitting a strong possibility.

To judge the classifier fairly, a much larger and more diverse set of images would be needed, ideally with more examples per class and a stronger architecture such as a CNN or transfer-learning approach.
